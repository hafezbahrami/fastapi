...
from model import UserList, UserEntry, UserDelete, UserUpdate
from pg_db import database, usersTableObj
...
async def find_user_by_id(userId: str):
    query = usersTableObj.select().\
            where(usersTableObj.c.id == userId)
    return await database.fetch_one(query)

@app.put("/users", response_model=UserList, tags=["Users"])
async def update_user(user: UserUpdate):
    gDate = str(datetime.datetime.now())
    query = usersTableObj.update().\
        where(usersTableObj.c.id == user.id).\
        values(
            first_name = user.first_name,
            last_name  = user.last_name,
            gender     = user.gender,
